...
#############################
# this is for children between 2 and 19.9 years of age (24 to 239.9 months)
# computes z-score, modified z-score, extended BMIz (and percentiles)
# and other BMI metrics based on the CDC growth charts

# Note - need sex coded as 1 (boys) or 2 (girls)
# Also need months of age, weight (kg), and height (cm)
# The names of age, weight and height can be anything in your data
# Also need the reference data for the CDC growth charts for the LMS parameters

# refdata_dir (line 81) is folder that contains 'CDCref_d.csv' -
#   this file can be download from
# https://www.cdc.gov/nccdphp/dnpao/growthcharts/resources/sas.htm
# (4th paragraph, before 'Instructions for SAS Users)

# specify the folder/directory of CDCref_d.csv in the ref_dir argument
# For example, ref_dir = '~/R/Growth_Charts/Data/'

# About age in months:
# If there are decimal place in months of age, keep all digits
# If only completed months of age is known (as in NHANES data),
#   specify 'integer=TRUE' (default is F) when calling the function
#   In this case, 0.5 will be added to the number of completed months
# if age is in years w decimal places, multiply by 12
# if age is in days, convert to months:  age_days/30.4375


# To use function:
# First, source this script/function

# Usage examples:
# For completed (integer) age in months (as in NHANES):
#    new <- ext_bmiz(yourdata_name, age=ridageex, integer=TRUE,
#                                wt=bmxwt, ht=bmxht, bmi=bmxbmi,
#                                ref_dir='[path_toCDCref_d.csv]/')
# Or for age in months with decimals omit the 'integer=T' or use 'integer=F':
#    d <- ext_bmiz(yourdata_name, age=agemos,
#                              wt=weight,ht=height, bmi=bmi,
#                              ref_dir='[path_toCDCref_d.csv]/')
# Or, d <- ext_bmiz(yourdata,agemos,wt,ht,bmi, ref_dir= 'whatever')

# agemos, weight, height and bmi are the names of these variables in your data
# When you run the function, do not put the variable names in quotation marks
...
# d$age <- d$agem
# data <- d
# ref_data <- fread('~/Sync/R/Anal/Growth_Charts/Data/CDCref_d.csv')
# data
# =========
...
anthro <- function(data, age=age_in_months, wt=weight_kg, ht=height_cm, bmi=bmi)
{
   age_in_months <- weight <- height <- seq_ <- sex <- agey <-
      lwt2 <- mwt2 <- swt2 <- lbmi2 <- mbmi2 <- sbmi2 <- lht2 <- mht2 <- sht2 <-
      lwt1 <- mwt1 <- swt1 <- lbmi1 <- mbmi1 <- sbmi1 <- lht1 <- mht1 <- sht1 <-
      mbmi <- lbmi <- sbmi <- mref <- sref <-
   ...
   v=c('seq_', 'bmiz', 'bmip', 'waz', 'wap', 'haz', 'hap', 'p50', 'p95', 'p97', 'bmip95',
       ...
       'adj_perc', 'perc_median', 'log_perc_median', 'aperc0')
   dt <- dt[,..v]
